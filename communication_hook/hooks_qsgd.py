...
class correlation_state(object):  # Equivalent to memory
    ...
    def get_mask(self,rank,base_grad: torch.Tensor, local_grad_list: list, bucket_index: int):
        ...
        for grad in local_grad_list:
            ...
            if len(size) == 4:
                vec = self.velocities[bucket_index + start: bucket_index + start + length].reshape (-1) 
                vec_q = quantize(vec,input_compress_settings={'n':8})   # Choose the quantization bit-width, which in this case is 4 bits.   
