    ...
    def get_mask(self,rank,base_grad: torch.Tensor, local_grad_list: list, bucket_index: int):
        ...
        for grad in local_grad_list:
            ...
            if len(size) == 4:
                ...
                c= math.ceil(torch.abs(vec).numel()*32*0.032)
                b = math.floor(0.5 * math.log2(2*math.log(2)*(c)) )
                k = math.floor((c)/ (0.5 * math.log2(2*math.log(2)*(c))))
                _, indices = torch.topk(torch.abs(vec), k)
                vec_k = torch.zeros_like(vec)
                vec_k[indices] = vec[indices]
                vec_q = quantize(vec_k, input_compress_settings={'n':2**(b-1)})         
